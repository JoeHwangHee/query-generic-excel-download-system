...
## 설정
- DB 접속: `src/main/resources/db.properties`
...
## 주요 흐름
1. **쿼리 저장** — 쿼리명/SQL(`?` 변수)/배치조건 지정, `?`별 형식(TEXT·NUMBER·DATE) 설정
2. **쿼리 실행** — 형식에 맞는 입력칸 동적 생성(DATE는 달력), 백그라운드 실행
...
ui/       MainFrame, QuerySaveDialog, QueryRunDialog, BatchAlertFlow
model/    QueryDef, ParamDef, QueryResult, RunHistory, enum(ParamType/DateFormatType/ScheduleType)
service/  QueryStore/QueryExecute/ExcelExport/Schedule/RunHistory/OutputPath
db/       DataSourceProvider(Hikari), GenericQueryRunner(JDBC)
